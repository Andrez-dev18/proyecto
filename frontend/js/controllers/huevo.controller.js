class HuevoController {
    ...
    async cargarCatalogos() {
        try {
            ...
            const [provincias, tipos, mercados, proveedores] = await Promise.all([
            ...
            this.catalogos = { provincias, tipos, mercados, proveedores };
    ...
    async aplicarFiltros() {
        try {
            ...
            const mercadoId = document.getElementById('filterMercado').value;
            ...
            if (mercadoId) filtros.mercado = parseInt(mercadoId);
            if (proveedorId) filtros.proveedor = parseInt(proveedorId);

            console.log('📤 Filtros a aplicar:', filtros);
            
            if (!fechaInicio && !fechaFin && !provinciaId && !tipoId && !mercadoId && !proveedorId) {
    ...
    cargarDatosEnFormulario() {
        ...
        const mercadoObj = this.catalogos.mercados.find(m => m.nombre === r.mercado);
    ...
    obtenerDatosFormulario() {
        ...
        const mercadoId = document.getElementById('modalMercado').value;
        ...
        return {
            ...
            mercado: parseInt(mercadoId) || null,
...
window.huevoController = new HuevoController();
