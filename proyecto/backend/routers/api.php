...
// ========== DASHBOARD ==========
if (strpos($path, "/dashboard/resumen-general") !== false && $request == "GET") {
    $dashboardController->getResumenGeneral();
    exit;
}

if (strpos($path, "/dashboard/vivo-aqp/resumen") !== false && $request == "GET") {
    $dashboardController->getVivoAqpResumen();
    exit;
}

if (strpos($path, "/dashboard/vivo-aqp/zona") !== false && $request == "GET") {
    $dashboardController->getVivoAqpPorZona();
    exit;
}

if (strpos($path, "/dashboard/vivo-aqp/proveedor") !== false && $request == "GET") {
    $dashboardController->getVivoAqpPorProveedor();
    exit;
}

if (strpos($path, "/dashboard/beneficiado/resumen") !== false && $request == "GET") {
    $dashboardController->getBeneficiadoProvinciaResumen();
    exit;
}

if (strpos($path, "/dashboard/trozado/resumen") !== false && $request == "GET") {
    $dashboardController->getTrozadoDiarioResumen();
    exit;
}

if (strpos($path, "/dashboard/trozado/producto") !== false && $request == "GET") {
    $dashboardController->getTrozadoPorProducto();
    exit;
}

if (strpos($path, "/dashboard/cliente/top") !== false && $request == "GET") {
    $dashboardController->getClienteProcesadosTop();
    exit;
}

if (strpos($path, "/dashboard/cliente/linea") !== false && $request == "GET") {
    $dashboardController->getClientePorLinea();
    exit;
}

if (strpos($path, "/dashboard/huevo/resumen") !== false && $request == "GET") {
    $dashboardController->getHuevoResumen();
    exit;
}

if (strpos($path, "/dashboard/gallina/resumen") !== false && $request == "GET") {
    $dashboardController->getGallinaResumen();
    exit;
}
// Criadores Emprendedores
if (preg_match('#^/dashboard/criadores/resumen$#', $uri)) {
    $dashboardController->getCriadoresEmprendedoresResumen();
    exit;
}

// Producto Sustituto
if (preg_match('#^/dashboard/producto-sustituto/resumen$#', $uri)) {
    $dashboardController->getProductoSustitutoResumen();
    exit;
}
