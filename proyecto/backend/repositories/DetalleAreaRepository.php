...
class DetalleAreaRepository
{
    ...
    // Obtener usuario de la sesión (sin llamar session_start si ya está iniciada)
    if (session_status() === PHP_SESSION_NONE) {
        session_start();
    }
    
    $usuarioSistema = isset($_SESSION['usuario']) ? $_SESSION['usuario'] : 
                     (isset($_SESSION['username']) ? $_SESSION['username'] : 
                     (isset($_SESSION['user']) ? $_SESSION['user'] : 'sistema'));
    
    if (!empty($data['id'])) {
        // ACTUALIZAR
        $query = "UPDATE com_db_detalle_area 
                  SET fecha = :fecha, 
                      provincia = :provincia, 
                      area = :area, 
                      motivo = :motivo, 
        ...
        $params = array(
            ...
            'usuarioReporte' => isset($data['usuarioReporte']) ? $data['usuarioReporte'] : null,
            'usuarioSistema' => $usuarioSistema
        );
        $stmt->execute($params);
        return $data['id'];
        ...
        $query = "INSERT INTO com_db_detalle_area 
                  (fecha, provincia, area, motivo, usuarioReporte, usuarioSistema) 
                  VALUES (:fecha, :provincia, :area, :motivo, :usuarioReporte, :usuarioSistema)";
